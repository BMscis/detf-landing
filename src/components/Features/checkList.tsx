...
const List = ({ text }) => (
  <p className="mb-5 flex items-center text-lg font-medium text-primary dark:text-body-color">
...
export default function CheckList({ lists }: { lists: string[] }) {
  ...
  const list1 = t.map((l) => <List text={l} />);
  const list2 = t2.map((l) => <List text={l} />);
  return (
